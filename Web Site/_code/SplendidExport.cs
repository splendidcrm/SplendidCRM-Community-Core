...
namespace SplendidCRM
{
	...
	public class SplendidExport
	{
		...
		public void Export(DataView vw, string sModuleName, string sExportFormat, string sExportRange, int nCurrentPage, int nPageSize, string[] arrID, bool bCustomPaging)
		{
			int nStartRecord = 0;
			...
			try
			{
				...
				switch ( sExportFormat )
				{
					case "csv"  :
						...
						Response.Headers.Add("Content-Disposition", "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".csv"));
						ExportDelimited(Response.Body, vw, sModuleName, nStartRecord, nEndRecord, ',' );
						//Response.End();
						Response.Body.Flush();
						Response.Body.Close();
						break;
					...
					case "xml"  :
						...
						// 08/06/2008 yxy21969.  Make sure to encode all URLs. 
						// 12/20/2009 Paul.  Use our own encoding so that a space does not get converted to a +. 
						Response.Headers.Add("Content-Disposition", "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".xml"));
						ExportXml(Response.Body, vw, sModuleName, nStartRecord, nEndRecord);
						//Response.End();
						Response.Body.Flush();
						Response.Body.Close();
						break;
					...
					default     :
						...
						// 08/06/2008 yxy21969.  Make sure to encode all URLs. 
						// 12/20/2009 Paul.  Use our own encoding so that a space does not get converted to a +. 
						Response.Headers.Add("Content-Disposition", "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".xlsx"));
						ExportExcelOpenXML(Response.Body, vw, sModuleName, nStartRecord, nEndRecord);
						//Response.End();
						Response.Body.Flush();
						Response.Body.Close();
						break;
		...
		public string OpenXML_GetColumnName(int nColumnIndex)
		{
		...
		public void OpenXML_RemoveText(SharedStringTablePart shareStringPart, string text, Dictionary<string, int> dictStringToInt)
		{
		...
		// 11/29/2013 Paul.  Use dictionary to increase performance. 
		public int OpenXML_InsertSharedStringItem(SharedStringTablePart shareStringPart, string text, Dictionary<string, int> dictStringToInt)
		{
			if ( shareStringPart.SharedStringTable == null )
			{
		...
		// 11/29/2013 Paul.  Use dictionary to increase performance. 
		public DocumentFormat.OpenXml.Spreadsheet.Cell OpenXML_CreateText(int nColumnIndex, int nRowIndex, SharedStringTablePart shareStringPart, string sText, Dictionary<string, int> dictStringToInt)
		{
		...
		public DocumentFormat.OpenXml.Spreadsheet.Cell OpenXML_CreateNumber(int nColumnIndex, int nRowIndex, string sText, UInt32Value styleId)
		{
			DocumentFormat.OpenXml.Spreadsheet.Cell cell = new DocumentFormat.OpenXml.Spreadsheet.Cell();
			cell.CellReference = OpenXML_GetColumnName(nColumnIndex) + nRowIndex.ToString();
		...
		public DocumentFormat.OpenXml.Spreadsheet.Cell OpenXML_CreateDate(int nColumnIndex, int nRowIndex, DateTime dtValue, UInt32Value styleId)
		{
			DocumentFormat.OpenXml.Spreadsheet.Cell cell = new DocumentFormat.OpenXml.Spreadsheet.Cell();
			cell.CellReference = OpenXML_GetColumnName(nColumnIndex) + nRowIndex.ToString();
		...
		public DocumentFormat.OpenXml.Spreadsheet.Cell OpenXML_CreateBoolean(int nColumnIndex, int nRowIndex, string sText)
		{
			DocumentFormat.OpenXml.Spreadsheet.Cell cell = new DocumentFormat.OpenXml.Spreadsheet.Cell();
			cell.CellReference = OpenXML_GetColumnName(nColumnIndex) + nRowIndex.ToString();
		...
		public Stylesheet OpenXML_CreateStylesheet()
		{
		...
		public Dictionary<string, string> GetExportTranslatedLists(string sModuleName)
		{
			Dictionary<string, string> dict = new Dictionary<string, string>();
			if ( Context != null )
			{
				// 09/08/2020 Paul.  This feature is configurable with the default being off. 
				if ( Sql.ToBoolean(Application["CONFIG.export_translate_lists"]) )
				{
		...
		public string TranslateExportedvalue(Dictionary<string, string> dict, L10N L10n, string sDATA_FIELD, DataRowView row)
		{
		...
		public void ExportExcelOpenXML(Stream stmResponse, DataTable dt, string[] arrHeaders, string sTitle, string sSubTitle1, string sSubTitle2)
		{
			// http://msdn.microsoft.com/en-us/library/office/ff478153.aspx
			// http://msdn.microsoft.com/en-us/library/office/cc850837
			using ( MemoryStream stm = new MemoryStream() )
