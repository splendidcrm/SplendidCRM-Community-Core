...
namespace SplendidCRM
{
	...
	public partial class SplendidCache
	{
		...
		// 04/30/2023 Paul.  Use reflection to get the keys from MemoryCache. 
		...
		{
			DataTable dt = memoryCache.Get("vwSqlColumns_Reporting." + sMODULE_NAME) as DataTable;
			if ( dt == null )
			{
				try
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								memoryCache.Set("vwSqlColumns_Reporting." + sMODULE_NAME, dt, DefaultCacheExpiration());
							}
						}
					}
				}
		...
		{
			string sLIST_NAME = memoryCache.Get("vwSqlColumns_ListName." + sMODULE_NAME + "." + sDATA_FIELD) as string;
			if ( sLIST_NAME == null )
			{
				try
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							memoryCache.Set("vwSqlColumns_ListName." + sMODULE_NAME + "." + sDATA_FIELD, sLIST_NAME, DefaultCacheExpiration());
						}
					}
				}
				catch(Exception ex)
		...
		public DataTable InboundEmailBounce()
		{
			...
			DataTable dt = memoryCache.Get("vwINBOUND_EMAILS_Bounce") as DataTable;
			if ( dt == null )
			{
				try
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								dt = new DataTable();
								da.Fill(dt);
								memoryCache.Set("vwINBOUND_EMAILS_Bounce", dt, DefaultCacheExpiration());
							}
						}
					}
				}
		...
		{
			// 02/16/2008 Paul.  When called from the scheduler, the context is not available, so get the cache from HttpRuntime. 
			DataTable dt = memoryCache.Get("vwINBOUND_EMAILS_Monitored") as DataTable;
			if ( dt == null )
			{
				try
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								dt = new DataTable();
								da.Fill(dt);
								memoryCache.Set("vwINBOUND_EMAILS_Monitored", dt, DefaultCacheExpiration());
							}
						}
					}
				}
