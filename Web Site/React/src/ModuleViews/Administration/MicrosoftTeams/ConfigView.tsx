...
const MODULE_NAME: string = 'MicrosoftTeams';

interface IMicrosoftTeamsConfigViewProps extends RouteComponentProps<any>
{
	callback?         : Function;
...
interface IMicrosoftTeamsConfigViewState
{
	item                : any;
	layout              : any;
	EDIT_NAME           : string;
...
class MicrosoftTeamsConfigView extends React.Component<IMicrosoftTeamsConfigViewProps, IMicrosoftTeamsConfigViewState>
{
	...
	constructor(props: IMicrosoftTeamsConfigViewProps)
	{
		super(props);
		let EDIT_NAME   : string = MODULE_NAME + '.ConfigView';
		let BUTTON_NAME : string = MODULE_NAME + '.ConfigView';
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_MICROSOFTTEAMS_SETTINGS');
	...
	async componentDidUpdate(prevProps: IMicrosoftTeamsConfigViewProps)
	{
		...
		try
		{
			...
			if ( this._isMounted )
			{
				...
				else if ( !Sql.IsEmptyString(queryParams['code']) )
				{
					...
					let res  = await CreateSplendidRequest('Administration/MicrosoftTeams/Rest.svc/GetAccessToken', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.Save token', json.d);
					item['MicrosoftTeams.OAuthAccessToken' ] = json.d.access_token ;
					item['MicrosoftTeams.OAuthRefreshToken'] = json.d.refresh_token;
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'MicrosoftTeams.OAuthAccessToken' , json.d.access_token );
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'MicrosoftTeams.OAuthRefreshToken', json.d.refresh_token);
					this.setState({ item, editedItem: item, error: L10n.Term('OAuth.LBL_TEST_SUCCESSFUL') });
					//this.props.history.replace('/Administration/MicrosoftTeams/ConfigView');
				...
				case 'Authorize':
				{
					...
					if ( this.Save() )
					{
						...
						try
						{
							...
							let client_id      : string = item['MicrosoftTeams.ClientID' ];
							...
							let scope          : string = "openid offline_access Team.ReadBasic.All Channel.ReadBasic.All ChannelMessage.ReadWrite Chat.Read.All";
							// 04/30/2021 Paul.  Instead of requiring /Administration/Exchange/ConfigView as a redirect, just bounce through Office365OAuth. 
							let state          : string = 'MicrosoftTeams';
							let redirect_url   : string = window.location.origin;
							redirect_url += window.location.pathname.replace(this.props.location.pathname, '');
							redirect_url += '/Office365OAuth';
							// 02/04/2023 Paul.  Directory Tenant is now required for single tenant app registrations. 
							let tenant         : string = item['MicrosoftTeams.DirectoryTenantID'];
	...
	private _onSelectDatabase = (database) =>
	{
		...
		if ( this._isMounted )
		{
			item['MicrosoftTeams.DefaultDatabaseID'  ] = database.ID  ;
			item['MicrosoftTeams.DefaultDatabaseName'] = database.NAME;
			this.setState( {error: null, item, isOpenSelectDatabase: false} );
			SplendidDynamic_EditView.SetRefValue(this.refMap, 'MicrosoftTeams.DefaultDatabaseID'  , database.ID  );
			SplendidDynamic_EditView.SetRefValue(this.refMap, 'MicrosoftTeams.DefaultDatabaseName', database.NAME);
		}
	}
