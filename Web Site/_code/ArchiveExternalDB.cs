...
namespace SplendidCRM
{
	...
	public class ArchiveExternalDB
	{
		...
		private DbAcrhiveFactories   DbAcrhiveFactories  = new DbAcrhiveFactories();
		...
		private string SqlDataType(string sDATA_TYPE, string sCHARACTER_MAXIMUM_LENGTH, string sNUMERIC_PRECISION, string sNUMERIC_SCALE)
		{
			string sCOLUMN_TYPE = String.Empty;
			switch ( sDATA_TYPE )
			{
		...
		private string BuildArchiveTable(string sTABLE_NAME)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				if ( bEXISTS )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						DbProviderFactory dbfArchive = DbAcrhiveFactories.GetFactory();
			...
			sb.Append(SqlBuildArchiveIndexes(sTABLE_NAME));
			if ( sb.Length > 0 )
				sb.AppendLine();
			return sb.ToString();
		}

		private string SqlBuildArchiveIndexes(string sTABLE_NAME)
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using ( IDbConnection con = dbf.CreateConnection() )
		...
		private string ArchiveBuildAll()
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using ( IDbConnection con = dbf.CreateConnection() )
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							foreach ( DataRow row in dt.Rows )
							{
								...
								sb.Append(BuildArchiveTable(sTABLE_NAME  ));
								sb.Append(BuildArchiveTable(sCUSTOM_TABLE));
								sb.Append(BuildArchiveTable(sAUDIT_TABLE ));
								sb.Append(BuildArchiveTable(sCUSTOM_AUDIT));
								...
								using ( IDbCommand cmdRelated = con.CreateCommand() )
								{
									...
									using ( DataTable dtRelated = new DataTable() )
									{
										...
										foreach ( DataRow rowRelated in dtRelated.Rows )
										{
											...
											sb.Append(BuildArchiveTable(sRELATED_NAME));
										}
									}
								}
							}
		...
		private string BuildInsertProcedures(string sTABLE_NAME, ref IDbCommand cmdPrimaryInsert, ref IDbCommand cmdCustomInsert)
		{
			sTABLE_NAME = sTABLE_NAME.ToUpper();
			string sCUSTOM_TABLE   = sTABLE_NAME  + "_CSTM";
		...
		private string ArchiveMoveData(Guid gID, string sTABLE_NAME, IDbCommand cmdPrimaryInsert, IDbCommand cmdCustomInsert)
		{
		...
		private string ArchiveMoveAuditData(Guid gAUDIT_ID, string sTABLE_NAME, IDbCommand cmdPrimaryInsert, IDbCommand cmdCustomInsert)
		{
			if ( cmdPrimaryInsert == null )
				return String.Empty;
		...
		private string ArchiveMoveRelationshipData(Guid gID, string sMODULE_NAME, string sTABLE_NAME)
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using ( IDbConnection con = dbf.CreateConnection() )
								...
								if ( sRELATED_TABLE.EndsWith("_AUDIT") )
								{
									...
									using ( IDbCommand cmd = con.CreateCommand() )
									{
										...
										using ( DataTable dtRelatedData = new DataTable() )
										{
											...
											BuildInsertProcedures(sRELATED_TABLE, ref cmdPrimaryInsert, ref cmdCustomInsert);
											...
											if ( cmdPrimaryInsert != null )
											{
												foreach ( DataRow row in dtRelatedData.Rows )
												{
													...
													sb.Append(ArchiveMoveAuditData(gAUDIT_ID, sRELATED_TABLE, cmdPrimaryInsert, cmdCustomInsert));
												}
											}
										}
									}
											...
											using ( DataTable dtRelatedData = new DataTable() )
											{
												...
												if ( cmdPrimaryInsert != null )
												{
													foreach ( DataRow row in dtRelatedData.Rows )
													{
														...
														sb.Append(ArchiveMoveData(gRELATED_ID, sRELATED_TABLE, cmdPrimaryInsert, cmdCustomInsert));
													}
												}
											}
										}
...
//										SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "Ignoring " + sRELATED_TABLE + " as it does not have an ID field.");
#endif
									}
								}
							}
		...
		private string ArchiveMoveActivitiesData(Guid gID, string sMODULE_NAME, string sTABLE_NAME)
		{
			IDbCommand cmdNOTE_ATTACHMENTSPrimaryInsert = null;
			IDbCommand cmdNOTE_ATTACHMENTSCustomInsert  = null;
			BuildInsertProcedures("NOTE_ATTACHMENTS", ref cmdNOTE_ATTACHMENTSPrimaryInsert, ref cmdNOTE_ATTACHMENTSCustomInsert);
			IDbCommand cmdEMAIL_IMAGESPrimaryInsert = null;
			IDbCommand cmdEMAIL_IMAGESCustomInsert  = null;
			BuildInsertProcedures("EMAIL_IMAGES", ref cmdEMAIL_IMAGESPrimaryInsert, ref cmdEMAIL_IMAGESCustomInsert);
			IDbCommand cmdNOTESPrimaryInsert = null;
			IDbCommand cmdNOTESCustomInsert  = null;
			BuildInsertProcedures("NOTES", ref cmdNOTESPrimaryInsert, ref cmdNOTESCustomInsert);

			IDbCommand cmdNOTE_ATTACHMENTS_AUDITPrimaryInsert = null;
			IDbCommand cmdNOTE_ATTACHMENTS_AUDITCustomInsert  = null;
			BuildInsertProcedures("NOTE_ATTACHMENTS", ref cmdNOTE_ATTACHMENTS_AUDITPrimaryInsert, ref cmdNOTE_ATTACHMENTS_AUDITCustomInsert);
			IDbCommand cmdEMAIL_IMAGES_AUDITPrimaryInsert = null;
			IDbCommand cmdEMAIL_IMAGES_AUDITCustomInsert  = null;
			BuildInsertProcedures("EMAIL_IMAGES_AUDIT", ref cmdEMAIL_IMAGES_AUDITPrimaryInsert, ref cmdEMAIL_IMAGES_AUDITCustomInsert);
			IDbCommand cmdNOTES_AUDITPrimaryInsert = null;
			IDbCommand cmdNOTES_AUDITCustomInsert  = null;
			BuildInsertProcedures("NOTES_AUDIT", ref cmdNOTES_AUDITPrimaryInsert, ref cmdNOTES_AUDITCustomInsert);
								...
								using ( IDbCommand cmd = con.CreateCommand() )
								{
									...
									using ( DataTable dtRelatedData = new DataTable() )
									{
										...
										if ( cmdPrimaryInsert != null )
										{
													...
													using ( IDbCommand cmdImages = con.CreateCommand() )
													{
														...
														using ( DataTable dtImagesData = new DataTable() )
														{
															...
															foreach ( DataRow rowImage in dtImagesData.Rows )
															{
																...
																sb.Append(ArchiveMoveData(gIMAGE_ID, sIMAGES_TABLE, cmdNOTE_ATTACHMENTSPrimaryInsert, cmdNOTE_ATTACHMENTSCustomInsert));
															}
														}
													}
												}
												else if ( sRELATED_TABLE == "EMAILS" && cmdEMAIL_IMAGESPrimaryInsert != null )
												{
													...
													using ( IDbCommand cmdImages = con.CreateCommand() )
													{
														...
														using ( DataTable dtImagesData = new DataTable() )
														{
															...
															{
																Guid gIMAGE_ID = Sql.ToGuid(rowImage["ID"]);
																sb.Append(ArchiveMoveData(gIMAGE_ID, sIMAGES_TABLE, cmdEMAIL_IMAGESPrimaryInsert, cmdEMAIL_IMAGESCustomInsert));
															}
														}
													}
													...
													using ( IDbCommand cmdNotes = con.CreateCommand() )
													{
														...
														using ( DataTable dtNotesData = new DataTable() )
														{
															...
															foreach ( DataRow rowNote in dtNotesData.Rows )
															{
																...
																sb.Append(ArchiveMoveData(gNOTE_ID, sNOTES_TABLE, cmdNOTESPrimaryInsert, cmdNOTESCustomInsert));
											...
											BuildInsertProcedures(sRELATED_AUDIT, ref cmdAuditPrimaryInsert, ref cmdAuditCustomInsert);
											// 04/12/2018 Paul.  If table does not exist on both main DB and archive DB, then the command will be null. 
													...
													using ( IDbCommand cmdAudit = con.CreateCommand() )
													{
														...
														using ( DataTable dtAuditData = new DataTable() )
														{
															...
															foreach ( DataRow rowAudit in dtAuditData.Rows )
															{
																...
																sb.Append(ArchiveMoveAuditData(gAUDIT_ID, sRELATED_AUDIT, cmdAuditPrimaryInsert, cmdAuditCustomInsert));
																	...
																	using ( IDbCommand cmdImages = con.CreateCommand() )
																	{
																		...
																		using ( DataTable dtImagesData = new DataTable() )
																		{
																			...
																			foreach ( DataRow rowImage in dtImagesData.Rows )
																			{
																				...
																				sb.Append(ArchiveMoveAuditData(gIMAGE_AUDIT_ID, sIMAGES_AUDIT_TABLE, cmdNOTE_ATTACHMENTS_AUDITPrimaryInsert, cmdNOTE_ATTACHMENTS_AUDITCustomInsert));
																			}
																		}
																	}
																}
																else if ( sRELATED_TABLE == "EMAILS_AUDIT" && cmdEMAIL_IMAGES_AUDITPrimaryInsert != null )
																{
																	...
																	using ( IDbCommand cmdImages = con.CreateCommand() )
																	{
																		...
																		using ( DataTable dtImagesData = new DataTable() )
																		{
																			...
																			{
																				Guid gIMAGE_AUDIT_ID = Sql.ToGuid(rowImage["AUDIT_ID"]);
																				sb.Append(ArchiveMoveAuditData(gIMAGE_AUDIT_ID, sIMAGES_AUDIT_TABLE, cmdEMAIL_IMAGES_AUDITPrimaryInsert, cmdEMAIL_IMAGES_AUDITCustomInsert));
																			}
																		}
																	}
																	// 04/14/2018 Paul.  Emails also contain notes/attachments. 
																	...
																	using ( IDbCommand cmdNotes = con.CreateCommand() )
																	{
																		...
																		using ( DataTable dtNotesData = new DataTable() )
																		{
																			...
																			foreach ( DataRow rowNote in dtNotesData.Rows )
																			{
																				...
																				sb.Append(ArchiveMoveAuditData(gNOTE_AUDIT_ID, sNOTES_AUDIT_TABLE, cmdNOTES_AUDITPrimaryInsert, cmdNOTES_AUDITCustomInsert));
		...
		private string ArchiveMoveRelatedData(Guid gID, string sMODULE_NAME, string sTABLE_NAME)
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using ( IDbConnection con = dbf.CreateConnection() )
								...
								if ( bEXISTS )
								{
									...
									using ( IDbCommand cmd = con.CreateCommand() )
									{
										...
										using ( DataTable dtRelatedData = new DataTable() )
										{
											...
											IDbCommand cmdPrimaryInsert = null;
											IDbCommand cmdCustomInsert  = null;
											BuildInsertProcedures(sRELATED_TABLE_NAME, ref cmdPrimaryInsert, ref cmdCustomInsert);
											IDbCommand cmdAuditPrimaryInsert = null;
											IDbCommand cmdAuditCustomInsert  = null;
											BuildInsertProcedures(sRELATED_AUDIT, ref cmdAuditPrimaryInsert, ref cmdAuditCustomInsert);
											// 04/12/2018 Paul.  If table does not exist on both main DB and archive DB, then the command will be null. 
											if ( cmdPrimaryInsert != null )
											{
												foreach ( DataRow row in dtRelatedData.Rows )
												{
													...
													sb.Append(ArchiveMoveActivitiesData  (gRELATED_ID, sRELATED_MODULE_NAME, sRELATED_TABLE_NAME));
													// 04/09/2018 Paul.  Prevent a recursive loop. 
													if ( sRELATED_MODULE_NAME != sMODULE_NAME )
														sb.Append(ArchiveMoveRelatedData     (gRELATED_ID, sRELATED_MODULE_NAME, sRELATED_TABLE_NAME));
													sb.Append(ArchiveMoveRelationshipData(gRELATED_ID, sRELATED_MODULE_NAME, sRELATED_TABLE_NAME));
													...
													sb.Append(ArchiveMoveData(gRELATED_ID, sRELATED_TABLE_NAME, cmdPrimaryInsert, cmdCustomInsert));
												}
		...
		private string ArchiveRulesRunAll()
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using ( IDbConnection con = dbf.CreateConnection() )
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dtRules = new DataTable() )
						{
							...
							foreach ( DataRow rowRule in dtRules.Rows )
							{
								...
								int nMaxArchiveBatchSize = Sql.ToInteger(Application["CONFIG.Archive.BatchSize"]);
								...
								using ( DataTable dt = new DataTable() )
								{
...
#endif
									SplendidError.SystemMessage("Information", new StackTrace(true).GetFrame(0), "ArchiveExternalDB.ArchiveRulesRunAll " + sTABLE_NAME + " with " + dt.Rows.Count.ToString() + " records.");
									
									string sAUDIT_TABLE = sTABLE_NAME + "_AUDIT";
									IDbCommand cmdPrimaryInsert = null;
									IDbCommand cmdCustomInsert  = null;
									BuildInsertProcedures(sTABLE_NAME, ref cmdPrimaryInsert, ref cmdCustomInsert);
									IDbCommand cmdAuditPrimaryInsert = null;
									IDbCommand cmdAuditCustomInsert  = null;
									BuildInsertProcedures(sAUDIT_TABLE, ref cmdAuditPrimaryInsert, ref cmdAuditCustomInsert);
									...
									if ( cmdPrimaryInsert != null )
									{
										for ( int i = 0; i < dt.Rows.Count && i < nMaxArchiveBatchSize; i++ )
										{
											...
											sb.Append(ArchiveMoveActivitiesData  (gID, sMODULE_NAME, sTABLE_NAME));
											sb.Append(ArchiveMoveRelatedData     (gID, sMODULE_NAME, sTABLE_NAME));
											sb.Append(ArchiveMoveRelationshipData(gID, sMODULE_NAME, sTABLE_NAME));
											// 04/12/2018 Paul.  Also move audit data. 
											// 04/12/2018 Paul.  If table does not exist on both main DB and archive DB, then the command will be null. 
											if ( cmdAuditPrimaryInsert != null )
											{
												...
												using ( IDbCommand cmdAudit = con.CreateCommand() )
												{
													...
													using ( DataTable dtAuditData = new DataTable() )
													{
														...
														{
															Guid gAUDIT_ID = Sql.ToGuid(rowAudit["AUDIT_ID"]);
															sb.Append(ArchiveMoveAuditData(gAUDIT_ID, sAUDIT_TABLE, cmdAuditPrimaryInsert, cmdAuditCustomInsert));
														}
													}
												}
											}
											// 04/11/2018 Paul.  Move main record last just in case there is an error. 
											sb.Append(ArchiveMoveData(gID, sTABLE_NAME, cmdPrimaryInsert, cmdCustomInsert));
										}
									}
								}
							}
...
#pragma warning disable CS1998
		public async ValueTask RunArchive(CancellationToken token)
		{
			RunArchive();
		}
#pragma warning restore CS1998
		...
		private void RunArchive()
		{
			if ( !bInsideArchive )
			{
				...
				try
				{
					SplendidError.SystemMessage("Information", new StackTrace(true).GetFrame(0), "ArchiveExternalDB.RunArchive Begin");
					...
					{
						SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to connect to the external archive database: " + Utils.ExpandException(ex));
						return;
					}
					if ( !Sql.IsEmptyString(Application["ArchiveConnectionString"]) )
					{
						if ( DateTime.Now > dtLastBuild.AddHours(6) )
						{
							...
							string sBuildSQL = ArchiveBuildAll();
							TimeSpan tsBuild = DateTime.Now - dtLastBuild;
							SplendidError.SystemMessage("Information", new StackTrace(true).GetFrame(0), "ArchiveExternalDB.RunArchive Build Complete in " + tsBuild.TotalMinutes.ToString() + " minutes");
						}
						//Debug.Write(sBuildSQL);
						string sMoveSQL = ArchiveRulesRunAll();
				...
				{
					SplendidError.SystemMessage("Information", new StackTrace(true).GetFrame(0), "ArchiveExternalDB.RunArchive End");
