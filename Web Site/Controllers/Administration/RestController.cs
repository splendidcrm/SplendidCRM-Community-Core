...
namespace SplendidCRM.Controllers.Administration
{
	...
	public class RestController : ControllerBase
	{
		...
		[HttpPost("[action]")]
		public async void RecompileViews()
		{
			if ( !Security.IsAuthenticated() || !Security.IS_ADMIN )
			{
				throw(new Exception(L10n.Term("ACL.LBL_INSUFFICIENT_ACCESS")));
		...
		[HttpPost("[action]")]
		public async void RebuildAudit()
		{
			if ( !Security.IsAuthenticated() || !Security.IS_ADMIN )
			{
				throw(new Exception(L10n.Term("ACL.LBL_INSUFFICIENT_ACCESS")));
...
// 10/31/2021 Paul.  All modules are available to the React client. 
		...
		[HttpPost("[action]")]
		public async void UndeleteModule([FromBody] Dictionary<string, object> dict)
		{
		...
		[HttpPost("[action]")]
		public async void DeleteAdminEditCustomField(Guid ID)
		{
		...
		[HttpPost("[action]")]
		public async void InsertAdminEditCustomField([FromBody] Dictionary<string, object> dict)
		{
			string ModuleName = "EditCustomFields";
			// 08/22/2011 Paul.  Add admin control to REST API. 
			// 01/04/2020 Paul.  SplendidCache.RestTables() does not typically include entries for admin tables, so we use Security.AdminUserAccess(). 
		...
		[HttpPost("[action]")]
		public async void UpdateAdminEditCustomField([FromBody] Dictionary<string, object> dict)
		{
			string ModuleName = "EditCustomFields";
			// 08/22/2011 Paul.  Add admin control to REST API. 
			// 01/04/2020 Paul.  SplendidCache.RestTables() does not typically include entries for admin tables, so we use Security.AdminUserAccess(). 
