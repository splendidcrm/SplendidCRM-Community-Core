...
namespace SplendidCRM
{
	...
	public class SplendidImport
	{
		...
		public SplendidImport(IWebHostEnvironment hostingEnvironment, IHttpContextAccessor httpContextAccessor, HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, SplendidError SplendidError, SplendidCache SplendidCache, SplendidDynamic SplendidDynamic)
		{
		...
		private void LogError(ref StringBuilder sbErrors, string sCommand, string sMessage)
		{
		...
		public void Import(XmlDocument xml, ArrayList arrTables, bool bTruncate)
		{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bTruncate )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								{
									da.Fill(dt);
									for ( int i = 0 ; i < dt.Rows.Count && Context.RequestAborted.IsCancellationRequested ; i++ )
									{
										DataRow row = dt.Rows[i];
			...
			for ( int i = 0 ; i < arrTables.Count && Context.RequestAborted.IsCancellationRequested ; i++ )
			{
				...
				if ( hashTables.ContainsKey(sTABLE_NAME) )
				{
					...
					if ( nlRows.Count > 0 )
					{
						...
						// Write a singe byte to keep the connection open. 
						Response.WriteAsync(" ");
						Response.BodyWriter.FlushAsync();
					}
				}
		...
		public void ZipSaveStream(Stream stm, string sTempPathName)
		{
		...
		public XmlDocument ConvertTableToXml(DataTable dt, string sRecordName)
		{
			return ConvertTableToXml(dt, sRecordName, false, false, null);
		}

		public XmlDocument ConvertTableToXml(DataTable dt, string sRecordName, bool bWithHeader, bool bUseColumnName, List<String> arrIgnoreColumns)
		{
		...
		public XmlDocument ConvertTextToXml(string sRecordName, Stream stm, char chFieldSeparator)
		{
			int nMaxField = 0;
			XmlDocument xml = new XmlDocument();
			xml.AppendChild(xml.CreateProcessingInstruction("xml" , "version=\"1.0\" encoding=\"UTF-8\""));
		...
		public XmlDocument ConvertXmlSpreadsheetToXml(XmlDocument xml, string sRecordName)
		{
		...
		public XmlDocument ConvertStreamToXml(string sImportModule, string sSourceType, string sCustomDelimiterValue, Stream stm, string sFILE_EXT)
		{
			XmlDocument xml = new XmlDocument();
