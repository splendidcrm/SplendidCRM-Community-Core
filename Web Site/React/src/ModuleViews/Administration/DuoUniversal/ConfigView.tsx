...
const MODULE_NAME: string = 'DuoUniversal';

interface IDuoUniversalConfigViewProps extends RouteComponentProps<any>
{
	callback?         : Function;
...
interface IDuoUniversalConfigViewState
{
	item                : any;
	layout              : any;
	EDIT_NAME           : string;
...
@observer
class DuoUniversalConfigView extends React.Component<IDuoUniversalConfigViewProps, IDuoUniversalConfigViewState>
{
	...
	constructor(props: IDuoUniversalConfigViewProps)
	{
		super(props);
		let EDIT_NAME   : string = MODULE_NAME + '.ConfigView';
		let BUTTON_NAME : string = MODULE_NAME + '.ConfigView';
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_DUO_UNIVERSAL_SETTINGS');
	...
	async componentDidUpdate(prevProps: IDuoUniversalConfigViewProps)
	{
	...
	private LoadItem = async (layout: any) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				let sREDIRECT_URL: string = Credentials.sREMOTE_SERVER + 'React/Administration/DuoUniversal/ConfigView';
				item['DuoUniversal.RedirectUrl'] = sREDIRECT_URL;
				...
				{
					let state: string = queryParams['state'];
					...
					let res  = await CreateSplendidRequest('Administration/DuoUniversal/Rest.svc/Verify', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					
					console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.LoadItem', json.d);
					if ( json.d === 'allow' )
					{
					}
					this.setState({ error: json.d });
					//this.props.history.replace('Administration/DuoUniversal/ConfigView');
	...
	private Save = async() =>
	{
		...
		try
		{
			...
			if ( Sql.IsEmptyString(row[MODULE_NAME + '.ClientSecret']) )
			{
				delete row[MODULE_NAME + '.ClientSecret'];
			}
...
export default withRouter(DuoUniversalConfigView);
