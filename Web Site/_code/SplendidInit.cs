	...
	public class SplendidInit
	{
		...
		public void InitAppURLs()
		{
			if ( Sql.IsEmptyString(Application["imageURL"]) )
			{
				...
				// https://stackoverflow.com/questions/40347426/in-asp-net-core-core-how-to-i-get-the-web-sites-ip-address
				string sServerName      = Request.Host.Host;  // Request.ServerVariables["SERVER_NAME"];
				// 01/14/2008 Paul.  Capture the IP Address as it is harder to get inside a scheduled task. 
				// https://www.bugsncodes.com/2019/03/how-to-get-client-ip-address-in-net.html
				string sServerIPAddress = Context.Connection?.LocalIpAddress.ToString();  // Request.ServerVariables["LOCAL_ADDR" ];
				// 01/04/2022 Paul.  Request.PathBase instead of hostingEnvironment.ApplicationName as application name will not change. 
				string sApplicationPath = Request.PathBase;  // Request.ApplicationPath;
				if ( !sApplicationPath.StartsWith("/") )
				...
				Application["ServerScheme"       ] = Request.Scheme  ;
				...
				bool IsDefaultPort = (Request.Scheme == "http" && Request.Host.Port == 80) || (Request.Scheme == "https" && Request.Host.Port == 443);
				Application["ServerPort"         ] = (IsDefaultPort ? "" : ":" + Request.Host.Port.ToString());
...
// 10/31/2021 Paul.  All modules are available to the React client. 
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader rdr = cmd.ExecuteReader() )
						{
							while ( rdr.Read() )
							{
								...
								// 09/26/2017 Paul.  Add Archive access right. 
								Security.SetModuleAccess(sMODULE_NAME, "archive", Sql.ToInteger(rdr["ACLACCESS_ARCHIVE"]));
							}
						}
					}
				}
		...
		public void InitApp()
		{
			//Console.WriteLine("SplendidInit.InitApp");
			...
			try
			{
				...
				if ( Application.Count < 20 )
					SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "Application start. Version " + Sql.ToString(Application["SplendidVersion"]));
				else
					SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "Application restart. Version " + Sql.ToString(Application["SplendidVersion"]));

				// 11/28/2005 Paul.  Clear all cache variables as well. 
				// 02/18/2008 Paul.  Cache. is a better and faster way to get to the cache. 
				SplendidCache.ClearCache(memoryCache);
				// 12/19/2021 Paul.  Session may not exist. 
				try
				{
				...
				{
					// 12/19/2021 Paul.  We can ignore this error as the session will not likely exist when app is being loaded. 
				}
				...
				{
					string sConfig = section.Key;
					Application["CONFIG." + sConfig] = Sql.ToString(section.Value);
				}
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					if ( Sql.IsSQLServer(con) )
					{
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							{
								bSQLAzure = true;
								SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "Connected to Microsoft SQL Azure.");
							}
							...
							if ( ! bSQLAzure )
							{
								try
								{
									...
									{
										bSQLAzure = true;
										SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "Connected to Microsoft SQL Server, but using SYSTEM_TRANSACTIONS table.");
									}
								}
								catch(Exception ex)
								{
				...
				{
					SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "Silverlight is disabled on Mono.");
		...
		public XmlDocument InitUserPreferences(string sUSER_PREFERENCES)
		{
			...
			try
			{
				...
				if ( Sql.IsEmptyString(sTimeZone) && !Sql.IsEmptyString(XmlUtil.SelectSingleNode(xml, "timez")) )
				{
					...
					sTimeZone = SplendidDefaults.TimeZone(SplendidCache, nTimez);
						...
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								...
								if ( !Sql.IsEmptyString(sUSER_PREFERENCES) )
								{
									...
									{
										SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Invalid USER_SETTINGS/TIMEZONE: " + XmlUtil.SelectSingleNode(xml, "timezone"));
									}
									try
									{
									...
									{
										SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Invalid USER_SETTINGS/CURRENCY: " + XmlUtil.SelectSingleNode(xml, "currency_id"));
									}
...
*/
					...
					using ( IDataReader rdr = cmd.ExecuteReader() )
					{
						...
						if ( rdr.Read() )
						{
							...
							catch(Exception ex)
							{
								...
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read TEAM_ID. " + ex.Message);
							...
							catch(Exception ex)
							{
								// 11/25/2006 Paul.  Ignore any team related issue as this error could prevent 
								// anyone from logging in.  The CRM would then be completely dead. 
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read EXCHANGE_ALIAS. " + ex.Message);
							...
							{
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read MAIL_SMTPUSER. " + ex.Message);
							...
							{
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read OFFICE365_OAUTH_ENABLED. " + ex.Message);
							...
							{
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read GOOGLEAPPS_OAUTH_ENABLED. " + ex.Message);
								//HttpContext.Current.Response.Write("Failed to read OFFICE365_OAUTH_ENABLED. " + ex.Message);
							}
							try
							{
							...
							{
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read IS_ADMIN_DELEGATE. " + ex.Message);
							...
							{
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read PRIMARY_ROLE_NAME. " + ex.Message);
							...
							{
								SplendidError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to read SYSTEM_GENERATED_PASSWORD. " + ex.Message);
							...
							if ( !Sql.IsEffiProz(con) )
								SqlProcs.spUSERS_LOGINS_InsertOnly(ref gUSER_LOGIN_ID, gUSER_ID, sUSER_NAME, sLOGIN_TYPE, "Succeeded", Session.Id, Context.Connection.RemoteIpAddress.ToString(), Request.Host.Value, Request.Path.Value, hostingEnvironment.WebRootPath, sUserAgent);
							Security.USER_LOGIN_ID = gUSER_LOGIN_ID;
							// 02/20/2011 Paul.  Log the success so that we can lockout the user. 
							this.LoginTracking(sUSER_NAME, true);
							...
							LoadTeams(gUSER_ID);
							bValidUser = true;
							SplendidError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "User login.");
						}
						...
						else
						{
							...
							string sUserAgent = Request.Headers["User-Agent"];
							if ( !Sql.IsEffiProz(con) )
								SqlProcs.spUSERS_LOGINS_InsertOnly(ref gUSER_LOGIN_ID, Guid.Empty, sUSER_NAME, sLOGIN_TYPE, "Failed", Session.Id, Context.Connection.RemoteIpAddress.ToString(), Request.Host.Value, Request.Path.Value, hostingEnvironment.WebRootPath, sUserAgent);
							// 02/20/2011 Paul.  Log the failure so that we can lockout the user. 
							this.LoginTracking(sUSER_NAME, false);
		...
		public void InitSession()
		{
			...
			try
			{
				...
				Session["USER_SETTINGS/CULTURE"] = SplendidDefaults.Culture();
				// 03/07/2007 Paul.  Version 1.4 moved its themes folder to the .NET 2.0 default App_Themes. 
				Session["themeURL"             ] = Sql.ToString(Application["rootURL"]) + "App_Themes/" + sTheme + "/";
