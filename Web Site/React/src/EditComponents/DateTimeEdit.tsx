...
export default class DateTimeEdit extends EditComponent<IEditComponentProps, IDateTimeEditState>
{
	...
	shouldComponentUpdate(nextProps: IEditComponentProps, nextState: IDateTimeEditState)
	{
	...
	{
		const { DATA_FIELD, DATA_VALUE, UI_REQUIRED, DATE_FORMAT, error } = this.state;
	...
	public render()
	{
		...
		try
		{
			...
			{
				return (<span>DATA_FIELD is empty for DateTimeEdit FIELD_INDEX { FIELD_INDEX }</span>);
			}
			else if ( onChanged == null )
			{
				return (<span>onChanged is null for DateTimeEdit DATA_FIELD { DATA_FIELD }</span>);
			}
			// 11/02/2019 Paul.  Hidden property is used to dynamically hide and show layout fields. 
