...
const MODULE_NAME: string = 'HubSpot';

interface IHubSpotConfigViewProps extends RouteComponentProps<any>
{
	callback?         : Function;
...
interface IHubSpotConfigViewState
{
	item                : any;
	layout              : any;
	EDIT_NAME           : string;
...
class HubSpotConfigView extends React.Component<IHubSpotConfigViewProps, IHubSpotConfigViewState>
{
	...
	constructor(props: IHubSpotConfigViewProps)
	{
		super(props);
		let EDIT_NAME   : string = MODULE_NAME + '.ConfigView';
		let BUTTON_NAME : string = MODULE_NAME + '.ConfigView';
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_HUBSPOT_SETTINGS');
		this.state =
		{
	...
	async componentDidUpdate(prevProps: IHubSpotConfigViewProps)
	{
	...
	private LoadItem = async (layout: any) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				...
				else if ( !Sql.IsEmptyString(queryParams['access_token']) )
				{
					let access_token : string = queryParams['access_token' ];
					let refresh_token: string = queryParams['refresh_token'];
					let expires_in   : number = Sql.ToInteger(queryParams['expires_in']);
					let expiresDate = new Date();
					expiresDate = new Date(expiresDate.valueOf() + expires_in * 1000);
					// 04/26/2015 Paul.  Convert expires seconds to expires time. 
					var sFormatString = Security.USER_DATE_FORMAT() + ' ' + Security.USER_TIME_FORMAT();
					let expires_at = formatDate(expiresDate, sFormatString);
					//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.LoadItem code', access_token);
					item['HubSpot.OAuthAccessToken' ] = access_token ;
					item['HubSpot.OAuthRefreshToken'] = refresh_token;
					item['HubSpot.OAuthExpiresAt'   ] = expires_at   ;
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'HubSpot.OAuthAccessToken' , access_token );
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'HubSpot.OAuthRefreshToken', refresh_token);
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'HubSpot.OAuthExpiresAt'   , expires_at   );
					this.setState({ item, editedItem: item, error: L10n.Term('HubSpot.LBL_CONNECTION_SUCCESSFUL') });
					//this.props.history.replace('/Administration/HubSpot/ConfigView');
				...
				case 'Authorize':
				{
					...
					if ( this.Save() )
					{
						...
						try
						{
							...
							let OAUTH_CLIENT_ID : string = item['HubSpot.ClientID' ];
							let OAUTH_PORTAL_ID : string = item['HubSpot.PortalID' ];
							let sREDIRECT_URL   : string = (window.location.origin + window.location.pathname);
							let authenticateUrl : string = 'https://app.hubspot.com/auth/authenticate?client_id=' + OAUTH_CLIENT_ID + '&portalId=' + OAUTH_PORTAL_ID + '&redirect_uri=' + sREDIRECT_URL + '&scope=contacts-rw+offline';
	...
	private _onSelectDatabase = (database) =>
	{
		...
		if ( this._isMounted )
		{
			item['HubSpot.DefaultDatabaseID'  ] = database.ID  ;
			item['HubSpot.DefaultDatabaseName'] = database.NAME;
			this.setState( {error: null, item, isOpenSelectDatabase: false} );
			SplendidDynamic_EditView.SetRefValue(this.refMap, 'HubSpot.DefaultDatabaseID'  , database.ID  );
			SplendidDynamic_EditView.SetRefValue(this.refMap, 'HubSpot.DefaultDatabaseName', database.NAME);
		}
	}
