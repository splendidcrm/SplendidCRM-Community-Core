...
const MODULE_NAME: string = 'iContact';

interface IiContactConfigViewProps extends RouteComponentProps<any>
{
	callback?         : Function;
...
interface IiContactConfigViewState
{
	item                : any;
	layout              : any;
	EDIT_NAME           : string;
...
class iContactConfigView extends React.Component<IiContactConfigViewProps, IiContactConfigViewState>
{
	...
	constructor(props: IiContactConfigViewProps)
	{
		super(props);
		let EDIT_NAME   : string = MODULE_NAME + '.ConfigView';
		let BUTTON_NAME : string = MODULE_NAME + '.ConfigView';
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_ICONTACT_SETTINGS');
	...
	async componentDidUpdate(prevProps: IiContactConfigViewProps)
	{
		...
		try
		{
			...
			switch (sCommandName)
			{
				...
				case 'GetAccount':
				{
					...
					CreateSplendidRequest('Administration/' + MODULE_NAME + '/Rest.svc/GetAccount', 'POST', 'application/octet-stream', sBody).then((res) =>
					{
						GetSplendidResult(res).then((json) =>
						{
							//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.GetAccount', json);
							item['iContact.iContactAccountId'     ] = json.d.AccountID     ;
							item['iContact.iContactClientFolderId'] = json.d.ClientFolderID;
							SplendidDynamic_EditView.SetRefValue(this.refMap, 'iContact.iContactAccountId'     , json.d.AccountID     );
							SplendidDynamic_EditView.SetRefValue(this.refMap, 'iContact.iContactClientFolderId', json.d.ClientFolderID);
							this.setState( {error: L10n.Term("iContact.LBL_ACCOUNT_RETRIEVED")} );
						})
						.catch((error) =>
						{
							this.setState({ error });
