...
const MODULE_NAME: string = 'Marketo';

interface IMarketoConfigViewProps extends RouteComponentProps<any>
{
	callback?         : Function;
...
interface IMarketoConfigViewState
{
	item                : any;
	layout              : any;
	EDIT_NAME           : string;
...
class MarketoConfigView extends React.Component<IMarketoConfigViewProps, IMarketoConfigViewState>
{
	...
	constructor(props: IMarketoConfigViewProps)
	{
		super(props);
		let EDIT_NAME   : string = MODULE_NAME + '.ConfigView';
		let BUTTON_NAME : string = MODULE_NAME + '.ConfigView';
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_MARKETO_SETTINGS');
	...
	async componentDidUpdate(prevProps: IMarketoConfigViewProps)
	{
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		try
		{
			...
			switch (sCommandName)
			{
				...
				{
					let item = this.data;
					let sBody: string = JSON.stringify(item);
					CreateSplendidRequest('Administration/' + MODULE_NAME + '/Rest.svc/Authorize', 'POST', 'application/octet-stream', sBody).then((res) =>
					{
						GetSplendidResult(res).then((json) =>
						{
							//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.Authorize', json);
							item['Marketo.OAuthAccessToken'] = json.d.access_token;
							item['Marketo.OAuthExpiresAt'  ] = json.d.expires_in  ;
							item['Marketo.OAuthScope'      ] = json.d.scope       ;
							SplendidDynamic_EditView.SetRefValue(this.refMap, 'Marketo.OAuthAccessToken', json.d.access_token);
							SplendidDynamic_EditView.SetRefValue(this.refMap, 'Marketo.OAuthExpiresAt'  , json.d.expires_in  );
							SplendidDynamic_EditView.SetRefValue(this.refMap, 'Marketo.OAuthScope'      , json.d.scope       );
							this.setState( {error: L10n.Term("Marketo.LBL_AUTHORIZE_SUCCESSFUL")} );
						})
						.catch((error) =>
						{
							this.setState({ error });
