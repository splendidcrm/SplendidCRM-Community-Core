...
// 112/19/2021 Paul.  Moved to separate class. 
#if false
		public IConfigurationSection AppSettings
		{
			get
			{
		...
		public static bool IsOfflineClient
		{
			get
			{
				// 12/19/2021 Paul.  The offline client is no longer supported. 
				return false;
			}
		}
		...
		public bool IsMobileDevice
		{
			get
			{
				...
				return Sql.ToBoolean(Session["IsMobileDevice"]);
			}
		}
		...
		public bool SupportsPopups
		{
			get
			{
				// 11/24/2010 Paul.  .NET 4 has broken the compatibility of the browser file system. 
				// We are going to minimize our reliance on browser files in order to reduce deployment issues. 
				return Sql.ToBoolean(Session["SupportsPopups"]);
			}
		}
		...
		public bool SupportsSpeech
		{
			get
			{
				return Sql.ToBoolean(Session["SupportsSpeech"]);
			}
		}
		...
		public bool SupportsHandwriting
		{
			get
			{
				return Sql.ToBoolean(Session["SupportsHandwriting"]);
			}
		}
		...
		public bool SupportsTouch
		{
			get
			{
				...
				return Sql.ToBoolean(Session["SupportsTouch"]);
			}
		}
		...
		public bool AllowAutoComplete
		{
			get
			{
				// 11/24/2010 Paul.  .NET 4 has broken the compatibility of the browser file system. 
				// We are going to minimize our reliance on browser files in order to reduce deployment issues. 
				return Sql.ToBoolean(Session["AllowAutoComplete"]);
			}
		}
...
#pragma warning restore CS1998
		...
		public DataTable CheckVersion()
		{
			...
			xml.Load(sVersionXmlURL + (Sql.ToBoolean(Application["CONFIG.send_usage_info"]) ? this.UsageInfo() : String.Empty));
		...
		public static string GenerateVCalendar(DataRow row, bool bIncludeAlarm)
		{
			...
			if ( oTimeZone == null )
				oTimeZone = Application["TIMEZONE.TZID.America/New_York"] as SplendidCRM.TimeZone;
			if ( oTimeZone == null )
			{
		...
		public static bool CachedFileExists(IWebHostEnvironment hostingEnvironment, string sVirtualPath)
		{
			HttpApplicationState Application = new HttpApplicationState();
			int nFileExists = Sql.ToInteger(Application["Exists." + sVirtualPath]);
			if ( nFileExists == 0 )
			{
				string sFullPath = Path.Combine(hostingEnvironment.ContentRootPath, sVirtualPath);  // Context.Server.MapPath(sVirtualPath)
				if ( File.Exists(sFullPath) )
					nFileExists = 1;
				else
					nFileExists = -1;
				Application["Exists." + sVirtualPath] = nFileExists;
			}
...
// 11/03/2021 Paul.  ASP.Net components are not needed. 
		...
		public static int DuplicateCheck(IDbConnection con, string sMODULE_NAME, Guid gID, SplendidControl ctl, DataRow rowCurrent)
		{
			int nDuplicates = 0;
	...
	public class ArchiveUtils
	{
		...
		public ArchiveUtils(HttpSessionState Session, Sql Sql, Security Security, Utils Utils, SplendidCache SplendidCache, SplendidError SplendidError, Crm.Modules Modules, IBackgroundTaskQueue taskQueue)
		{
			...
			if ( !Sql.IsEmptyString(Application["CONFIG.Archive.MaxBulkCount"]) )
			{
				this.nMaxBulkCount = Sql.ToInteger(Application["CONFIG.Archive.MaxBulkCount"]);
			}
		}
...
#pragma warning restore CS1998
		...
		private void MoveDataInternal()
		{
			try
			{
				...
				// 03/03/2021 Paul.  We need a version of FilterByACL_Stack() that does not require Context.Session so that it can be called from a Rest thread. 
				System.Collections.Stack stk = Utils.FilterByACL_Stack(this.ModuleName, this.nACLACCESS, arrID, Modules.TableName(this.ModuleName));
				if ( stk.Count > 0 )
				{
					DbProviderFactory dbf = DbProviderFactories.GetFactory();
					using ( IDbConnection con = dbf.CreateConnection() )
