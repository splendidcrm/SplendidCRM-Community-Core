...
import { Impersonate }                              from '../../../scripts/Login'               ;
...
@observer
class UsersAdminListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private _onImpersonateUser = async (row) =>
	{
		try
		{
			if ( window.confirm(L10n.Term('Users.NTC_CONFIRM_IMPERSONATION')) )
			{
				await Impersonate(row);
				this.props.history.push('/Reload');
			}
		}
		catch(error)
		{
			console.error((new Date()).toISOString() + ' ' + this.constructor.name + '._onImpersonateUser', error);
			this.setState({ error });
		}
	}
	...
	private exportColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		...
		return (<span>
			...
			? <span>
				<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onImpersonateUser(row) } className='listViewTdToolsS1'>
					<img src={ this.themeURL + 'users.gif' } style={ {borderWidth: '0px', height: '16px', width: '16px', paddingLeft: '4px'} } title={ L10n.Term('Users.LBL_IMPERSONATE') } />
					&nbsp;{ L10n.Term('Users.LBL_IMPERSONATE') }
				</span>
			</span>
			: null
			}
...
export default withRouter(UsersAdminListView);
