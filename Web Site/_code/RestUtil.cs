...
namespace SplendidCRM
{
	...
	public class RestUtil
	{
		...
		private SplendidCharge.CC    CC                 ;

		public RestUtil(IHttpContextAccessor httpContextAccessor, HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, Utils Utils, SplendidError SplendidError, SplendidCache SplendidCache, SplendidDynamic SplendidDynamic, SplendidInit SplendidInit, Crm.Images Images, ExchangeSync ExchangeSync, ChatManager ChatManager, PayTraceUtils PayTraceUtils, AuthorizeNetUtils AuthorizeNetUtils, PayPalRest PayPalRest)
		{
			...
			this.CC                  = new SplendidCharge.CC(Security, Sql, SqlProcs);
		}
		...
		public List<string> AccessibleModules()
		{
			List<string> lstMODULES = SplendidCache.AccessibleModules(Security.USER_ID);
			...
			*/
			...
			if ( Security.AdminUserAccess("SystemLog", "list") >= 0 )
			{
		...
		public DataTable GetTable(string sTABLE_NAME, int nSKIP, int nTOP, string sFILTER, string sORDER_BY, string sGROUP_BY, UniqueStringCollection arrSELECT, Guid[] arrITEMS, ref long lTotalCount, UniqueStringCollection arrAGGREGATE, AccessMode enumAccessMode, bool bArchiveView, string sDUPLICATE_FIELDS, StringBuilder sbDumpSQL)
		{
							...
							if ( dt != null )
							{
								...
								foreach(DataRow row in dt.Rows)
								{
									...
									if ( sMODULE_NAME != "Home" && sMODULE_NAME != "Activities" && sMODULE_NAME != "Dashboard" )
									{
										...
										{
											bool bVisible = (Security.GetUserAccess(sMODULE_NAME, "list") >= 0) && vwSYNC_TABLES.Count > 0;
											if ( !bVisible )
											{
												row.Delete();
												bRowsDeleted = true;
...
// 12/20/2021 TODO.  Make sure to use [ResponseCache(Duration = 0, Location = ResponseCacheLocation.None, NoStore = true)]
#if false
								...
								using ( IDbCommand cmd = con.CreateCommand() )
								{
									...
									{
										if ( Security.AdminUserAccess("Teams", "list") >= 0 )
										{
											Security.Filter(cmd, sMODULE_NAME, "view");
										}
										else if ( !Config.enable_team_hierarchy() )
										{
...
//#endif
									...
									using ( DbDataAdapter da = dbf.CreateDataAdapter() )
									{
										...
										if ( sTABLE_NAME == "CONFIG" )
										{
											...
											row["VALUE"       ] = Config.enable_data_privacy();
										}
		...
		public DataTable GetAdminTable(string sTABLE_NAME, int nSKIP, int nTOP, string sFILTER, string sORDER_BY, string sGROUP_BY, UniqueStringCollection arrSELECT, Guid[] arrITEMS, ref long lTotalCount, UniqueStringCollection arrAGGREGATE, AccessMode enumAccessMode, StringBuilder sbDumpSQL)
		{
			// 05/19/2018 Paul.  Capture the last command for error tracking. 
			string sLastCommand = String.Empty;
			DataTable dt = null;
		...
		public Guid UpdateTable(string sTABLE_NAME, Dictionary<string, object> dict)
		{
			return UpdateTable(sTABLE_NAME, dict, true);
		}

		// 06/21/2021 Paul.  Move bExcludeSystemTables to method parameter so that it can be used by admin REST methods. 
		public Guid UpdateTable(string sTABLE_NAME, Dictionary<string, object> dict, bool bExcludeSystemTables)
		{
			Guid gID = Guid.Empty;
			try
			{
				...
				foreach ( string sColumnName in dict.Keys )
				{
					...
					else if ( sColumnName != "SaveDuplicate" && sColumnName != "SaveConcurrency" && sColumnName != "LAST_DATE_MODIFIED" )
					{
						...
						if ( sTABLE_NAME == "USERS")
						{
							...
							{
								string sPASSWORD = Sql.ToString(dict[sColumnName]);
								if ( !Sql.IsEmptyString(sPASSWORD) && sPASSWORD != Sql.sEMPTY_PASSWORD && (Security.AdminUserAccess("Users", "edit") >= 0) )
								{
							...
							if ( String.Compare(sColumnName, "CARD_NUMBER") == 0 && dtUPDATE.Columns.Contains(sColumnName) )
							{
								...
								if ( Sql.ToBoolean(Application["CONFIG.PayTrace.Enabled"]) && !Sql.IsEmptyString(Application["CONFIG.PayTrace.UserName"]) )
								{
									if ( sCARD_NUMBER != Sql.sEMPTY_PASSWORD )
									{
										...
										PayTraceUtils.UpdateCustomerProfile(ref gID, sNAME, sCREDIT_CARD_NUMBER, sSECURITY_CODE, dtEXPIRATION_DATE.Month, dtEXPIRATION_DATE.Year, sADDRESS_STREET, sADDRESS_CITY, sADDRESS_STATE, sADDRESS_POSTALCODE, sADDRESS_COUNTRY, sEMAIL, sPHONE);
								...
								else if ( Sql.ToBoolean(Application["CONFIG.AuthorizeNet.Enabled"]) && !Sql.IsEmptyString(Application["CONFIG.AuthorizeNet.UserName"]) )
								{
									...
									if ( sCARD_NUMBER != Sql.sEMPTY_PASSWORD )
									{
										...
										string sCREDIT_CARD_NUMBER = sCARD_NUMBER;
										AuthorizeNetUtils.UpdateCustomerProfile(ref sCARD_TOKEN, sNAME, sCREDIT_CARD_NUMBER, sSECURITY_CODE, sBANK_NAME, sBANK_ROUTING_NUMBER, dtEXPIRATION_DATE, sADDRESS_STREET, sADDRESS_CITY, sADDRESS_STATE, sADDRESS_POSTALCODE, sADDRESS_COUNTRY, sEMAIL, sPHONE);
								...
								else if ( !Sql.IsEmptyString(Application["CONFIG.PayPal.ClientID"]) && !Sql.IsEmptyString(Application["CONFIG.PayPal.ClientSecret"]) )
								{
									...
									if ( sCARD_NUMBER != Sql.sEMPTY_PASSWORD )
									{
										...
										string sCREDIT_CARD_NUMBER = sCARD_NUMBER;
										// 12/15/2015 Paul.  Add EMAIL and PHONE for Authorize.Net. 
										PayPalRest.StoreCreditCard(ref sCARD_TOKEN, sNAME, sCARD_TYPE, sCREDIT_CARD_NUMBER, sSECURITY_CODE, dtEXPIRATION_DATE.Month, dtEXPIRATION_DATE.Year, sADDRESS_STREET, sADDRESS_CITY, sADDRESS_STATE, sADDRESS_POSTALCODE, sADDRESS_COUNTRY, sEMAIL, sPHONE);
										row["CARD_NUMBER"  ] = Security.EncryptPassword(sCARD_TOKEN, gCREDIT_CARD_KEY, gCREDIT_CARD_IV);
										row["SECURITY_CODE"] = String.Empty;
									}
								}
								...
								else
								{
									...
									if ( sCARD_NUMBER != Sql.sEMPTY_PASSWORD )
									{
										string sCARD_TOKEN         = String.Empty;
										string sCREDIT_CARD_NUMBER = sCARD_NUMBER;
										CC.StoreCreditCard(ref sCARD_TOKEN, sNAME, sCARD_TYPE, sCREDIT_CARD_NUMBER, sSECURITY_CODE, sBANK_NAME, sBANK_ROUTING_NUMBER, dtEXPIRATION_DATE.Month, dtEXPIRATION_DATE.Year, sADDRESS_STREET, sADDRESS_CITY, sADDRESS_STATE, sADDRESS_POSTALCODE, sADDRESS_COUNTRY, sEMAIL, sPHONE);
										row["CARD_NUMBER"        ] = Security.EncryptPassword(sCARD_TOKEN, gCREDIT_CARD_KEY, gCREDIT_CARD_IV);
										row["SECURITY_CODE"      ] = String.Empty;
				...
				if ( Security.IsAuthenticated() )
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						else if ( sTABLE_NAME == "TAGS" )
						{
							...
							if ( Sql.IsEmptyGuid(gID) && Security.AdminUserAccess("Tags", "edit") >= 0 )
							{
								bExcludeSystemTables = false;
							}
						}
																...
																foreach ( DataRow rowCustomField in dtCustomFields.Rows )
																{
																	...
																	if ( vwFiles.Table.Columns.Contains("DATA_FIELD") )
																	{
																		...
																		if ( vwFiles.Count > 0 )
																		{
																			...
																			Images.LoadFile(gImageID, byFILE_DATA, trn);
