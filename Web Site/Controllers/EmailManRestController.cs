...
namespace SplendidWebApi.Controllers
{
	...
	public class EmailManRestController : ControllerBase
	{
		...
		public EmailManRestController(IWebHostEnvironment hostingEnvironment, IMemoryCache memoryCache, HttpSessionState Session, Security Security, Utils Utils, SplendidError SplendidError, SplendidCache SplendidCache, RestUtil RestUtil, EmailUtils EmailUtils, ActiveDirectory ActiveDirectory, SplendidCRM.Crm.Modules Modules, IBackgroundTaskQueue taskQueue)
		{
		...
		public string Office365_Authorize([FromBody] Dictionary<string, object> dict)
		{
			...
			if ( Security.IsAuthenticated() && Security.AdminUserAccess("EmailMan", "edit") >= 0 )
			{
				if ( !Sql.IsEmptyString(sCode) )
				{
					...
					Office365AccessToken token = ActiveDirectory.Office365AcquireAccessToken(Context, sOAuthDirectoryTenatID, sOAuthClientID, sOAuthClientSecret, EmailUtils.CAMPAIGN_MANAGER_ID, sCode, sRedirectURL);
					// 01/19/2017 Paul.  We want an OUTBOUND_EMAILS mapping to the office365 OAuth record. 
		...
		public string Office365_Test([FromBody] Dictionary<string, object> dict)
		{
			...
			if ( Security.IsAuthenticated() && Security.AdminUserAccess("EmailMain", "edit") >= 0 )
			{
				...
				// 02/04/2023 Paul.  Directory Tenant is now required for single tenant app registrations. 
				string sOAuthDirectoryTenatID = Sql.ToString(Application["CONFIG.Exchange.DirectoryTenantID"]);
				ActiveDirectory.Office365TestAccessToken(Application, sOAuthDirectoryTenatID, sOAuthClientID, sOAuthClientSecret, EmailUtils.CAMPAIGN_MANAGER_ID, sbErrors);
		...
		public void Office365_RefreshToken([FromBody] Dictionary<string, object> dict)
		{
			...
			if ( Security.IsAuthenticated() && Security.AdminUserAccess("EmailMan", "edit") >= 0 )
			{
				...
				// 02/04/2023 Paul.  Directory Tenant is now required for single tenant app registrations. 
				string sOAuthDirectoryTenatID = Sql.ToString(Application["CONFIG.Exchange.DirectoryTenantID"]);
				ActiveDirectory.Office365RefreshAccessToken(Application, sOAuthDirectoryTenatID, sOAuthClientID, sOAuthClientSecret, EmailUtils.CAMPAIGN_MANAGER_ID, true);
			}
			else
			{
				throw(new Exception(L10n.Term("ACL.LBL_INSUFFICIENT_ACCESS")));
		...
		public void SendQueued([FromBody] Dictionary<string, object> dict)
		{
			...
			if ( Security.IsAuthenticated() && Security.AdminUserAccess("EmailMan", "edit") >= 0 )
			{
				...
				_taskQueue.QueueBackgroundWorkItemAsync(campaign.QueueStart);
