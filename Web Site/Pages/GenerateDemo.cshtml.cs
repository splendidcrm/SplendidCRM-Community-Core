...
{
	public class GenerateDemoModel : PageModel
	{
		...
		public GenerateDemoModel(IWebHostEnvironment hostingEnvironment, IHttpContextAccessor httpContextAccessor, IMemoryCache memoryCache, HttpSessionState Session, Security Security, SplendidError SplendidError, SplendidCache SplendidCache, SplendidInit SplendidInit)
		{
		...
		public void OnGet()
		{
			...
			{
				lblError = L10n.Term("ACL.LBL_NO_ACCESS");
				return;
			}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbTransaction trn = Sql.BeginTransaction(con) )
				{
					...
					if ( Security.IS_ADMIN && Sql.IsEmptyString(Application["CONFIG.Configurator.LastRun"]) && !Utils.IsOfflineClient )
						Context.Response.Redirect("~/Administration/Configurator/".Replace("~/", Sql.ToString(Application["rootURL"])));
					// 09/22/2010 Paul.  Redirect to the new User Wizard.  The user cannot be modified on the Offline Client. 
						...
						Response.Redirect("~/login".Replace("~/", Sql.ToString(Application["rootURL"]))); // Response.Redirect("~/Users/SetTimezone.aspx");
					else
