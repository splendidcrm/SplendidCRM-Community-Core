...
namespace SplendidCRM
{
	public class MimeUtils
	{
		...
		public string NormalizeInternetAddressName(MailboxAddress addr)
		{
		...
		public void BuildAddressList(InternetAddress to, StringBuilder sbTO_ADDRS, StringBuilder sbTO_ADDRS_NAMES, StringBuilder sbTO_ADDRS_EMAILS)
		{
		...
		public void BuildAddressIDList(IDbConnection con, InternetAddress to, StringBuilder sbTO_ADDRS_IDS, StringBuilder sbTO_ADDRS_NAMES, StringBuilder sbTO_ADDRS_EMAILS)
		{
			if ( to is MailboxAddress )
			{
				MailboxAddress addr = to as MailboxAddress;
		...
		public string GetAttachments(MimeMessage email)
		{
			...
			foreach ( MimeEntity att in email.Attachments )
			{
				...
				else if ( att is MimePart )
				{
					...
					if ( part.Content != null && part.Content.Stream != null )
						XmlUtil.SetSingleNode(xml, xAttachment, "Size"              , part.Content.Stream.Length.ToString());
					XmlUtil.SetSingleNode(xml, xAttachment, "MediaType"         , part.ContentType.MediaType     );
					XmlUtil.SetSingleNode(xml, xAttachment, "CharSet"           , part.ContentType.Charset       );
					XmlUtil.SetSingleNode(xml, xAttachment, "ContentType"       , part.ContentType.ToString()    );
					XmlUtil.SetSingleNode(xml, xAttachment, "ContentID"         , part.ContentId                 );
		...
		public string[] arrTrackers = new string[] { "/RemoveMe.aspx?identifier=", "/campaign_trackerv2.aspx?identifier=", "/image.aspx?identifier=", "X-SplendidCRM-ID: " };

		public Guid FindTargetTrackerKey(MimeMessage mm, string sHtmlBody, string sTextBody)
		{
		...
		public string EmbedInlineImages(MimeMessage email, string sDESCRIPTION_HTML)
		{
		...
		public Guid ImportInboundEmail(IDbConnection con, MimeMessage mm, Guid gMAILBOX_ID, string sINTENT, Guid gGROUP_ID, Guid gGROUP_TEAM_ID, string sUNIQUE_MESSAGE_ID)
		{
			...
			//{
				...
				if ( mm.To != null )
				{
					...
					{
						BuildAddressList(to, sbTO_ADDRS, sbTO_ADDRS_NAMES, sbTO_ADDRS_EMAILS);
					}
				}
				...
				if ( mm.Cc != null )
				{
					...
					{
						BuildAddressList(cc, sbCC_ADDRS, sbCC_ADDRS_NAMES, sbCC_ADDRS_EMAILS);
					}
				}
				...
				if ( mm.Bcc != null )
				{
					...
					{
						BuildAddressList(bcc, sbBCC_ADDRS, sbBCC_ADDRS_NAMES, sbBCC_ADDRS_EMAILS);
					}
				}
				...
				sSAFE_BODY_HTML = EmbedInlineImages(mm, sSAFE_BODY_HTML);
						...
						foreach ( MimeEntity att in mm.Attachments )
						{
							if ( att is MessagePart || att is MimePart )
							{
								...
								using ( MemoryStream mem = new MemoryStream() )
								{
									...
									NoteAttachments.LoadFile(gNOTE_ATTACHMENT_ID, mem, trn);
								}
							}
						}
						trn.Commit();
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( email.To != null )
					{
						...
						{
							BuildAddressIDList(con, to, sbTO_ADDRS_IDS, sbTO_ADDRS_NAMES, sbTO_ADDRS_EMAILS);
						}
					}
					...
					if ( email.Cc != null )
					{
						...
						{
							BuildAddressIDList(con, cc, sbCC_ADDRS_IDS, sbCC_ADDRS_NAMES, sbCC_ADDRS_EMAILS);
						}
					}
					...
					if ( email.Bcc != null )
					{
						...
						{
							BuildAddressIDList(con, bcc, sbBCC_ADDRS_IDS, sbBCC_ADDRS_NAMES, sbBCC_ADDRS_EMAILS);
						}
					}
					// 01/21/2017 Paul.  Only get the body values once as they may be computed. 
					// http://www.mimekit.net/docs/html/WorkingWithMessages.htm
					...
					sDESCRIPTION_HTML = EmbedInlineImages(email, sDESCRIPTION_HTML);
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
						{
		...
		public DataRow CreateMessageRecord(DataTable dt, MimeMessage email, double dSize)
		{
			...
			else if ( email.Date != DateTimeOffset.MinValue )
			{
			...
			// 04/30/2023 Paul.  email.Date cannot be null. 
			if ( email.Date.DateTime != DateTime.MinValue )
			{
