...
const MODULE_NAME: string = 'Watson';

interface IWatsonConfigViewProps extends RouteComponentProps<any>
{
	callback?         : Function;
...
interface IWatsonConfigViewState
{
	item                : any;
	layout              : any;
	EDIT_NAME           : string;
...
@observer
class WatsonConfigView extends React.Component<IWatsonConfigViewProps, IWatsonConfigViewState>
{
	...
	constructor(props: IWatsonConfigViewProps)
	{
		super(props);
		let EDIT_NAME   : string = MODULE_NAME + '.ConfigView';
		let BUTTON_NAME : string = MODULE_NAME + '.ConfigView';
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_WATSON_SETTINGS');
	...
	async componentDidUpdate(prevProps: IWatsonConfigViewProps)
	{
	...
	private LoadItem = async (layout: any) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				item['Watson.DefaultDatabaseName'] = Sql.ToString(item['Watson.DefaultDatabaseID']);
				...
				else if ( !Sql.IsEmptyString(queryParams['code']) )
				{
					...
					let res  = await CreateSplendidRequest('Administration/Watson/Rest.svc/GetAccessToken', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.Save token', json);
					item['Watson.OAuthRefreshToken'] = json.d;
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'Watson.OAuthRefreshToken', json.d);
					this.setState({ item, editedItem: item, error: L10n.Term('Watson.LBL_CONNECTION_SUCCESSFUL') });
					//this.props.history.replace('/Administration/Watson/ConfigView');
					this.props.history.replace('ConfigView');
				}
				else if ( !Sql.IsEmptyString(item['Watson.DefaultDatabaseID']) )
				{
					let sDatabaseID: string = Sql.ToString(item['Watson.DefaultDatabaseID']);
					let res  = await CreateSplendidRequest('Administration/' + MODULE_NAME + '/Rest.svc/GetDatabaseName?DatabaseID=' + sDatabaseID, 'GET', 'application/json; charset=utf-8', null);
					let json = await GetSplendidResult(res);
					if ( json != null )
					{
						item['Watson.DefaultDatabaseName'] = json.d;
						SplendidDynamic_EditView.SetRefValue(this.refMap, 'Watson.DefaultDatabaseName', json.d);
						this.setState({ layout, item, editedItem: item });
					}
				}
			}
	...
	private Save = async() =>
	{
		...
		Object.keys(this.refMap).map((key) =>
		{
			...
			if ( data )
			{
				// 11/09/2019 Paul.  Watson.DefaultDatabaseName is computed and not stored. 
				if ( data.key == 'Watson.DefaultDatabaseName' )
				{
				}
		...
		try
		{
			...
			switch (sCommandName)
			{
				...
				case 'Authorize':
				{
					...
					if ( this.Save() )
					{
						...
						try
						{
							...
							let OAUTH_CLIENT_ID : string = item['Watson.ClientID' ];
							let OAUTH_REGION    : string = item['Watson.Region'   ];
							let OAUTH_POD_NUMBER: string = item['Watson.PodNumber'];
							...
							let authenticateUrl : string = 'https://api-campaign-' + OAUTH_REGION + '-' + OAUTH_POD_NUMBER + '.goacoustic.com/oauth/authorize?response_type=code&client_id=' + OAUTH_CLIENT_ID + '&redirect_uri=' + encodeURI(sREDIRECT_URL);
				...
				case 'RefreshToken':
				{
					...
					CreateSplendidRequest('Administration/' + MODULE_NAME + '/Rest.svc/RefreshToken', 'POST', 'application/octet-stream', sBody).then((res) =>
					{
						GetSplendidResult(res).then((json) =>
						{
				...
				case 'SelectDatabase':
				{
					CreateSplendidRequest('Administration/' + MODULE_NAME + '/Rest.svc/GetDatabaseList', 'GET', 'application/json; charset=utf-8', null).then((res) =>
					{
						GetSplendidResult(res).then((json) =>
						{
							//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.GetDatabaseList', json);
							this.setState( {error: null, databases: json.d, isOpenSelectDatabase: true} );
						})
						.catch((error) =>
						{
							this.setState({ error });
	...
	private _onSelectDatabase = (database) =>
	{
		...
		if ( this._isMounted )
		{
			item['Watson.DefaultDatabaseID'  ] = database.ID  ;
			item['Watson.DefaultDatabaseName'] = database.NAME;
			this.setState( {error: null, item, isOpenSelectDatabase: false} );
			SplendidDynamic_EditView.SetRefValue(this.refMap, 'Watson.DefaultDatabaseID'  , database.ID  );
			SplendidDynamic_EditView.SetRefValue(this.refMap, 'Watson.DefaultDatabaseName', database.NAME);
		}
	}
...
export default withRouter(WatsonConfigView);
